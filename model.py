...
def predict(orgno):
        ...
        print('link is successfully scrap off')
        a = []
        b = []
        x = 0
        ...
        for i in range(len(b)):
                if(x == int(b[i])):
                        ...
                        nameOfOrganization = soup.find(
                            'h3', class_="banner__title")
                        nameOfOrganization = nameOfOrganization.text
                        nameOfOrganization = re.sub(
                            "\s+", ' ', nameOfOrganization)
                        c.append(nameOfOrganization)

                        linkOfOrganization = soup.find('a', class_="org__link")
                        linkOfOrganization = linkOfOrganization.text
                        linkOfOrganization = re.sub(
                            "\s+", ' ', linkOfOrganization)
                        c.append(linkOfOrganization)

                        descOfOrganization = soup.find(
                            'h4', class_="org__tagline")
                        descOfOrganization = descOfOrganization.text
                        descOfOrganization = re.sub(
                            "\s+", ' ', descOfOrganization)
                        c.append(descOfOrganization)

                        techOfOrganization = soup.find(
                            'ul', class_="org__tag-container")
                        techOfOrganization = techOfOrganization.text
                        techOfOrganization = re.sub(
                            "\s+", ',', techOfOrganization)
